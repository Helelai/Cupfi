...
const DefaultRowActions = ({ index, item, getRowActions, getRowDescriptor, rowActionsSize, isRowDisabled, }) => {
    ...
    return (_jsxs("div", { className: actionsCn, children: [_jsx(Popup, { open: isPopupOpen, anchorRef: anchorRef, placement: DEFAULT_PLACEMENT, onOutsideClick: closePopup, id: rowId, children: _jsx(Menu, { className: menuCn, size: rowActionsSize, children: actions.map(renderPopupMenuItem) }) }), _jsx(Button, { view: "flat-secondary", className: actionsButtonCn, onClick: togglePopup, size: rowActionsSize, ref: anchorRef, disabled: disabled, "aria-label": i18n('label-actions'), "aria-expanded": isPopupOpen, "aria-controls": rowId, children: _jsx(Icon, { data: Ellipsis }) })] }));
};
export function withTableActions(TableComponent) {
    ...
    return class extends React.Component {
        ...
        enhanceColumns = _memoize((columns) => enhanceSystemColumn(columns, (systemColumn) => {
            systemColumn.template = this.renderBodyCell;
        }));
        // eslint-disable-next-line @typescript-eslint/member-ordering
        enhanceOnRowClick = _memoize((onRowClick) => {
            if (!onRowClick) {
                return onRowClick;
            }
